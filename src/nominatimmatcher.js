...
* @constructor
*/
kt.NominatimMatcher = function(opt_url, opt_payload) {
  ...
  * @private
  */
  this.url_ = opt_url || 'http://nominatim.klokantech.com/';
...
kt.NominatimMatcher.prototype.requestMatchingRows =
  ...
  if (this.request_ !== null) this.jsonp_.cancel(this.request_);
   ...
   //NLS HACK: Search in UK or World
   var nlsgazarea = document.querySelector('input[name="nlsgazarea"]:checked');
   if(nlsgazarea.value.length > 0){
     this.payload_['q'] = this.payload_['q'] + ', ' + nlsgazarea.value;
   }
